...
- **多语种界面**：界面现已支持 12 种语言——英语、简体/繁体中文、日语、韩语、俄语、意大利语、阿拉伯语（含从右到左布局）、法语、德语、西班牙语和葡萄牙语。语言跟随系统区域或 `MARK_SHOT_LANG` 环境变量，也可在 设置 → 通用 中即时切换。
- **语言即时切换**：在设置中切换界面语言会立即重新翻译整个设置窗口，无需重启。
- **未保存修改保护**：设置分类会显示未保存标记，页脚与标题提示存在待保存编辑；关闭时询问 保存并关闭 / 放弃并关闭 / 继续编辑（含 Escape 键路径）。
- **关于页面**：设置"高级"下方新增"关于"入口，展示公司 Logo、版本号、开源社区版链接，以及对原上游项目及其作者、贡献者的致谢。
- **设置滚轮防护**：在设置页悬停下拉框、数值框和滑块时滚动鼠标滚轮将滚动页面而不再篡改控件值；聚焦控件仍保留滚轮调整能力。
- **无头模式设置**：存储页可配置无头截图的默认去向（直接内联返回 base64 或暂存临时文件），并将剪贴板写入权限默认关闭、开启需输入固定口令二次确认。
- **分栏还原配置**：每个设置栏目与页面都提供"还原"按钮，一键回到最近一次已保存的配置；高级页另提供二次确认的"还原原始设置"。
- **快捷键录入防护**：快捷键输入框拒绝纯修饰键与危险键，全局快捷键要求至少一个修饰键或功能键，页面内重复快捷键会被提示。
- **设置页滚轮精确滚动**：高分辨率鼠标与触控板按比例滚动（分数增量跨事件累积，对齐原生 Qt），Ctrl/Shift + 滚轮整页滚动。
- **X11 全局快捷键布局健壮性**：Alt/Super/NumLock 掩码按实时键盘映射解析（修复 Alt/Super 互换等自定义布局），键盘映射变更后自动重新抓取；功能键支持扩展到 F25-F35。
- **用户手册支持 12 种语言**：操作手册现已提供全部支持语言的版本。
