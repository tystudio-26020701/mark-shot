...
- **捕获冻结范围配置支持**：引入配置项 `capture.freezeScope`，允许在多屏环境下设置区域截图的显示器冻结范围。支持全屏冻结（`all-screens`，默认）以及仅冻结鼠标指针所在屏幕（`cursor-screen`）。
- **贴图窗口架构解耦重构**：将原本位于 `shot_window_pinned_window.cpp` 中庞大复杂的匿名命名空间贴图类，拆分提取为 `src/pinned_window/` 目录下的多个专门子模块（包括 OCR、翻译、文本选择、几何以及拉伸缩放控制器），极大地提升了贴图功能的代码可读性与模块化设计。
- **启动配置与会话启动模块化**：将 `main.cpp` 中的环境变量注入、启动参数解析、默认工具及 Qt Portal 服务禁用等初始化流程模块化到 `startup_config`；并将截图窗口的创建和多屏冻结策略重构封装到独立的 `capture_session_launcher`。
- **测试用例扩展**：添加了针对捕获冻结范围（`capture_freeze_scope_test`）和贴图边界缩放控制器（`pinned_resize_controller_test`）的单元测试，保证重构及核心逻辑的正确性。
...
- **Wayland 贴图置顶原生支持**：为贴图窗口实现跨平台置顶管理器。针对 Wayland 环境原生支持 LayerShell Top 协议角色，支持动态角色重构、防失效延迟提升（Raise）机制，并于右键菜单集成“始终置顶”可配置选项。
- **Windows 图形捕获 (WGC) 后端**：新增基于 Windows Graphics Capture API 的高性能捕获后端，原生支持无边框捕获（去除阴影与边框）、多显示器虚拟屏幕计算，并解决了 MinGW (MSYS2/UCRT64) 构建兼容性。
- **GNOME Wayland 兼容性与窗口探测**：深度重构 GNOME Shell 滚动助手扩展，并新增专用辅助探测脚本，以提高 GNOME 桌面环境下窗口边界和坐标计算的准确性。
- **统一配置管理器与存储**：新增标准配置读写存储器（`app_config_store`），实现配置项的原子化更新，保障多平台配置持久化的健壮性。
- **优化标注交互工作流**：数字标注工具新增多种序号样式选择（阿拉伯数字、字母、罗马数字、中文数字、天干等）并支持一键重置计数；优化文本编辑器自动避让策略，防止文本框超出边界；启用文本编辑器输入法（IME）支持并改进长文本光标可见性。
- **可配置调试日志**：引入命令行参数 `--debug`、`--no-debug` 与 `--debug-log` 指明诊断 file，且支持在 `config.json` 中定义 `debug.enabled` 及 `debug.logPath`。
- **缺陷修复与调优**：修复 Windows 多显示器环境下滚动预览遮挡与可见性缺陷；修正 Windows 进程亲和性（Affinity）构建错误；优化 Wayland 叠加层坐标换算。

### 0.1.23

- **Windows 构建支持与托盘集成**：支持在 Windows (MSYS2/UCRT64) 下使用 Qt 6 进行编译。实现了系统托盘图标支持、全局热键注册、系统原生字体适配，以及针对多显示器环境的虚拟屏幕几何区域计算。
- **应用图标与托盘默认配置**：新增应用程序图标 `mark-shot.ico` 并更新系统托盘的默认配置选项。
- **代码注释与文档支持**：为核心模块与辅助子模块添加了详尽的代码注释，提升了系统架构的清晰度。
- **源码结构重构**：将体积过大的源码文件（如 `shot_window.cpp` 和 `screen_capture.cpp`）拆分为多个功能专一的子模块，以提高代码的可维护性与可测试性。

### 0.1.22

- **标注旋转与弯曲箭头**：引入标注组件旋转机制。支持旋转手柄，拖动即可对几何标注（如矩形、椭圆、文本等）进行任意角度旋转。同时增强了箭头工具，支持通过控制点将直线箭头调节为贝塞尔曲线弯曲箭头。
- **荧光笔样式与放大镜倍数**：荧光笔新增自由绘制与直线绘制模式，支持在属性面板快速切换；放大镜新增倍数调节滑块（默认 2.75），允许在属性面板自由调整放大比例。
- **滚动截图边框渲染优化**：修复 X11 环境下 Qt 窗口 Mask 裁剪导致截图边框不显示的问题，将重绘区域自动合并至 Input Mask 中，确保边框正常渲染的同时内部依然保持鼠标穿透。
- **滚动截图边缘伪影智能擦除**：设计了边缘杂质擦除算法（`scrubCaptureFrameArtifacts`），自动扫描并清除拼接时由边界框引入的杂质像素行或列，保证滚动长图接缝处的平滑与整洁。
- **滚动截图隐藏预览选项**：新增 `scrollCapture.hidePreviewDuringCapture` (`hidePreviewWhileCapturing`) 配置，开启后在捕获期间会隐藏预览面板以防阻挡视线，暂停时则自动恢复展示。
- **重构 X11 窗口边界检测算法**：优先通过 `_NET_CLIENT_LIST_STACKING` 或 `_NET_CLIENT_LIST` 属性读取窗口堆栈以优化探测性能；读取 `_NET_FRAME_EXTENTS` 使得获取的窗口几何区域精准包含修饰边框；通过读取 `_NET_WM_STATE` 过滤隐藏或处于最小化（Iconic）状态的窗口；排除 `override_redirect` 窗口。支持 `windowDetection.enabled` 全局开关。
