...
手动安装时，必须同时安装 `mark-shot`、`mark-shot-ocr`、`mark-shot-code-scan` 和 `mark-shot-translate`。否则 OCR、扫码或翻译功能无法调用后端脚本。
...
此命令会安装可执行文件、辅助脚本（`mark-shot-ocr`、`mark-shot-code-scan`、`mark-shot-translate`）、桌面快捷方式和图标。
