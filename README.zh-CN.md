...
主截图窗口里的 OCR 流程默认仍是“识别后直接复制到剪贴板”。

如果你更习惯先编辑再复制，优先可直接写到 `~/.config/mark-shot/config.json`：
...
也可以在启动前临时设置 shell 环境变量：

```bash
MARK_SHOT_OCR_RESULT_PANEL=1 mark-shot
```

可识别的开启值有 `1`、`true`、`yes`、`on`。开启后，主窗口 OCR 会弹出侧边结果面板，支持编辑、复制、翻译和拖动移动，而不是立刻复制。
