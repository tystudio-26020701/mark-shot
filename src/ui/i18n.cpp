...
const QHash<QString, QString> &chineseTable()
{
    static const QHash<QString, QString> table = {
        ...
        {QStringLiteral("Uses native grabs on X11 and the desktop portal on Wayland Linux desktops, and RegisterHotKey on Windows."),
         QStringLiteral("X11 使用原生按键抓取，Wayland Linux 桌面使用 desktop portal，Windows 使用 RegisterHotKey。")},
        ...
        {QStringLiteral("This key combination is not allowed as a shortcut."),
         QStringLiteral("该组合键不允许作为快捷键。")},
        {QStringLiteral("Press a key combination to assign, or leave empty to disable."),
         QStringLiteral("按下组合键以分配，或留空以禁用。")},
        {QStringLiteral("This shortcut is already assigned to another action."),
         QStringLiteral("该快捷键已分配给其他操作。")},
        {QStringLiteral("Global shortcuts need at least one modifier key or a function key."),
         QStringLiteral("全局快捷键至少需要一个修饰键或功能键。")},
        {QStringLiteral("Restore"), QStringLiteral("还原")},
        {QStringLiteral("Restore Page"), QStringLiteral("还原本页")},
        {QStringLiteral("Restore this section to the currently saved configuration."),
         QStringLiteral("将本栏目还原为当前已保存的配置。")},
        {QStringLiteral("Restore this page to the currently saved configuration."),
         QStringLiteral("将本页还原为当前已保存的配置。")},
        {QStringLiteral("Restore Original Settings"), QStringLiteral("还原原始设置")},
        {QStringLiteral("Reset every setting back to the factory defaults. This cannot be undone."),
         QStringLiteral("将所有设置还原为出厂默认值。此操作不可撤销。")},
        {QStringLiteral("Reset every setting to its factory default? This cannot be undone, and "
                        "any unsaved changes will be lost."),
         QStringLiteral("是否将所有设置还原为出厂默认值？此操作不可撤销，未保存的修改将丢失。")},
        ...
        {QStringLiteral("Control where window captures go when Mark Shot runs without a UI. "
                        "The clipboard is never modified unless you explicitly enable it here."),
         QStringLiteral("控制 Mark Shot 无界面运行时窗口截图结果的去向。除非在此显式开启，否则绝不修改剪贴板。")},
        ...
        {QStringLiteral("Permit headless window captures to write into the system clipboard. "
                        "Disabled by default; enabling requires a confirmation passphrase."),
         QStringLiteral("允许无头模式截图写入系统剪贴板。默认关闭；开启需输入确认口令。")},
        {QStringLiteral("Enable Headless Clipboard Access"), QStringLiteral("开启无头模式剪贴板权限")},
        {QStringLiteral("Headless mode may write into your system clipboard. "
                        "To confirm you understand the risk, type the passphrase "
                        "shown below exactly and press Confirm."),
         QStringLiteral("无头模式可能写入您的系统剪贴板。为确认您已了解风险，请逐字输入下方显示的口令并点击「确认」。")},
        ...
        {QStringLiteral("The passphrase does not match. Clipboard access stays disabled."),
         QStringLiteral("口令不匹配，剪贴板权限保持关闭。")},
