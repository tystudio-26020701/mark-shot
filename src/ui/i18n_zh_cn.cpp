...
const QHash<QString, QString> &tableZhCN()
{
    static const QHash<QString, QString> table = {
        ...
        {QStringLiteral("Enter inserts newline, click outside commits, Esc cancels"), QStringLiteral("回车换行，点击外部确认，Esc 取消")},
        ...
        {QStringLiteral("Color picker: left click samples a pixel   Right/Esc returns"), QStringLiteral("取色器：左键采样像素   右键/Esc 返回")},
        {QStringLiteral("Ruler: hover reads coordinates, drag measures pixels   Right/Esc returns"), QStringLiteral("尺子：悬停读取坐标，拖动测量像素   右键/Esc 返回")},
        ...
        {QStringLiteral("No extension commands configured.\nCreate %1"), QStringLiteral("未配置扩展命令。\n请创建 %1")},
        {QStringLiteral("Only one image file can be opened at a time."), QStringLiteral("一次只能打开一个图片文件。")},
        ...
        {QStringLiteral("OCR backend not installed. Install rapidocr or tesseract."), QStringLiteral("未安装 OCR 后端，请安装 rapidocr 或 tesseract。")},
        ...
        {QStringLiteral("Review or edit the recognized text before copying."), QStringLiteral("复制前可先检查或编辑识别结果。")},
        ...
        {QStringLiteral("Translating edited OCR text. Keep this panel open."), QStringLiteral("正在翻译编辑后的 OCR 文字。请保持此面板打开。")},
        {QStringLiteral("Scroll capture is not supported on GNOME Wayland."), QStringLiteral("GNOME Wayland 暂不支持滚动截图。")},
        ...
        {QStringLiteral("Some changes take effect after restarting Mark Shot."), QStringLiteral("部分设置需要重启 Mark Shot 后生效。")},
        {QStringLiteral("Settings saved. Some changes take effect after restarting Mark Shot."), QStringLiteral("设置已保存，部分设置需要重启 Mark Shot 后生效。")},
        ...
        {QStringLiteral("Configure tray startup and global shortcuts."), QStringLiteral("配置托盘启动和全局快捷键。")},
        {QStringLiteral("Configure interface language, tray startup, and global shortcuts."), QStringLiteral("配置界面语言、托盘启动和全局快捷键。")},
        {QStringLiteral("Configure interface language, theme, tray startup, and global shortcuts."), QStringLiteral("配置界面语言、主题、托盘启动和全局快捷键。")},
        ...
        {QStringLiteral("Launch Mark Shot directly into the system tray."), QStringLiteral("启动 Mark Shot 时直接进入系统托盘。")},
        {QStringLiteral("Global Hotkeys"), QStringLiteral("全局快捷键")},
        {QStringLiteral("Register global capture shortcuts when the tray starts."), QStringLiteral("托盘启动时注册全局截图快捷键。")},
        {QStringLiteral("Hotkeys"), QStringLiteral("快捷键")},
        {QStringLiteral("Uses native grabs on X11 and the desktop portal on Wayland Linux desktops, and RegisterHotKey on Windows."), QStringLiteral("X11 使用原生按键抓取，Wayland Linux 桌面使用 desktop portal，Windows 使用 RegisterHotKey。")},
        {QStringLiteral("Capture Hotkey"), QStringLiteral("截图快捷键")},
        {QStringLiteral("Fullscreen Hotkey"), QStringLiteral("全屏截图快捷键")},
        {QStringLiteral("Adjust how the frozen screenshot is captured before annotation starts."), QStringLiteral("调整标注开始前冻结截图的捕获方式。")},
        {QStringLiteral("Include Cursor"), QStringLiteral("包含鼠标指针")},
        {QStringLiteral("Capture the mouse cursor in the frozen image when supported."), QStringLiteral("平台支持时在冻结图中捕获鼠标指针。")},
        ...
        {QStringLiteral("Use KWin ScreenShot2 on KDE Wayland when available."), QStringLiteral("KDE Wayland 可用时使用 KWin ScreenShot2。")},
        {QStringLiteral("Hide Mark Shot Windows While Capturing"), QStringLiteral("截图时隐藏 Mark Shot 窗口")},
        {QStringLiteral("Hide own windows from screenshots. Turn off to include them."), QStringLiteral("截图时隐藏自身窗口。关闭即可保留。")},
        {QStringLiteral("Tool Shortcuts"), QStringLiteral("工具快捷键")},
        {QStringLiteral("Configure shortcuts used while editing a selected screenshot."), QStringLiteral("配置选中截图后的标注工具快捷键。")},
        {QStringLiteral("Action Shortcuts"), QStringLiteral("动作快捷键")},
        {QStringLiteral("Configure screenshot action shortcuts."), QStringLiteral("配置截图操作快捷键。")},
        {QStringLiteral("Startup Tool Shortcuts"), QStringLiteral("启动阶段工具快捷键")},
        {QStringLiteral("Configure shortcuts available before a region is selected."), QStringLiteral("配置选区前可用的工具快捷键。")},
        ...
        {QStringLiteral("Set the initial tools and color used by new capture windows."), QStringLiteral("设置新截图窗口使用的初始工具和颜色。")},
        ...
        {QStringLiteral("Change compact toolbar sizes used in capture windows."), QStringLiteral("调整截图窗口中紧凑工具栏的尺寸。")},
        ...
        {QStringLiteral("Control pinned image window behavior."), QStringLiteral("控制置顶图片窗口行为。")},
        {QStringLiteral("Keep pinned images above normal windows when the platform supports it."), QStringLiteral("平台支持时让置顶图片保持在普通窗口上方。")},
        {QStringLiteral("Allow selecting OCR or translated text with the mouse."), QStringLiteral("允许用鼠标拖选 OCR 或翻译文本。")},
        {QStringLiteral("Pinned Border"), QStringLiteral("置顶图片边框")},
        {QStringLiteral("Draw a border around pinned images."), QStringLiteral("在置顶图片周围绘制边框。")},
        ...
        {QStringLiteral("Configure automatic text recognition for pinned images."), QStringLiteral("配置置顶图片的自动文字识别。")},
        {QStringLiteral("OCR Enabled"), QStringLiteral("启用 OCR")},
        {QStringLiteral("Enable OCR actions for pinned images."), QStringLiteral("启用置顶图片 OCR 操作。")},
        {QStringLiteral("Auto OCR"), QStringLiteral("自动 OCR")},
        {QStringLiteral("Recognize text automatically after pinning."), QStringLiteral("钉住图片后自动识别文字。")},
        ...
        {QStringLiteral("Translate automatically after OCR completes."), QStringLiteral("OCR 完成后自动翻译。")},
        {QStringLiteral("Translation Command"), QStringLiteral("翻译命令")},
        {QStringLiteral("Translation Timeout"), QStringLiteral("翻译超时")},
        {QStringLiteral("Configure the floating frame and preview panel used for scrolling screenshots."), QStringLiteral("配置滚动截图使用的浮动边框和预览面板。")},
        {QStringLiteral("Capture Frame"), QStringLiteral("截图外框")},
        {QStringLiteral("Show an outer frame around the scrolling capture region."), QStringLiteral("在滚动截图区域外显示边框。")},
        ...
        {QStringLiteral("Hide the preview panel while each frame is captured."), QStringLiteral("每次捕获画面时隐藏预览面板。")},
        {QStringLiteral("Configure the external helper used to recognize QR codes and barcodes."), QStringLiteral("配置用于识别二维码和条形码的外部辅助程序。")},
        ...
        {QStringLiteral("Configure the external helper used to upload screenshots."), QStringLiteral("配置用于上传截图的外部辅助程序。")},
        ...
        {QStringLiteral("Configure OCR result panels and API-based translation helpers."), QStringLiteral("配置 OCR 结果面板和基于 API 的翻译辅助程序。")},
        {QStringLiteral("OCR Result Panel"), QStringLiteral("OCR 结果面板")},
        {QStringLiteral("Show an editable OCR result panel before copying text."), QStringLiteral("复制文字前显示可编辑的 OCR 结果面板。")},
        ...
        {QStringLiteral("Choose which provider Mark Shot uses for OCR, translation, and code scanning."), QStringLiteral("选择 Mark Shot 用于 OCR、翻译和扫码的 Provider。")},
        ...
        {QStringLiteral("Provider plugins are loaded from these directories when Mark Shot starts."), QStringLiteral("Mark Shot 启动时会从这些目录加载 Provider 插件。")},
        ...
        {QStringLiteral("Inspect loaded provider plugins and reasons for unavailable plugins."), QStringLiteral("查看已加载的 Provider 插件以及插件不可用原因。")},
        ...
        {QStringLiteral("No provider plugins were found in the configured search directories."), QStringLiteral("在已配置的搜索目录中没有找到 Provider 插件。")},
        {QStringLiteral("Translation"), QStringLiteral("翻译")},
        {QStringLiteral("Debug"), QStringLiteral("调试")},
        {QStringLiteral("Configure diagnostic logging for troubleshooting."), QStringLiteral("配置用于排障的诊断日志。")},
        ...
        {QStringLiteral("Configure the external helper used to detect windows under the selection."), QStringLiteral("配置用于检测选区下方窗口的外部辅助程序。")},
        {QStringLiteral("Window Detection Enabled"), QStringLiteral("启用窗口检测")},
        {QStringLiteral("Run the configured helper before region selection."), QStringLiteral("选区前运行已配置的辅助程序。")},
        ...
        {QStringLiteral("Environment variables applied when Mark Shot starts."), QStringLiteral("Mark Shot 启动时应用的环境变量。")},
        {QStringLiteral("Saving"), QStringLiteral("保存")},
        {QStringLiteral("Configure the default file name template for saved screenshots."), QStringLiteral("配置保存截图时的默认文件名模板。")},
        {QStringLiteral("Path Template"), QStringLiteral("路径模板")},
        {QStringLiteral("Recording Output"), QStringLiteral("录制输出")},
        {QStringLiteral("Configure where video and GIF recordings are saved."), QStringLiteral("配置视频和 GIF 录制文件的保存位置。")},
        ...
        {QStringLiteral("Choose how copied images are placed into the clipboard."), QStringLiteral("选择复制图片写入剪贴板的方式。")},
        ...
        {QStringLiteral("Add a macOS-style transparent canvas and soft shadow to shared screenshots."), QStringLiteral("为分享用截图添加类似 macOS 的透明画布和柔和阴影。")},
        {QStringLiteral("Mac-style Frame"), QStringLiteral("Mac 风格外框")},
        {QStringLiteral("Apply only to saved, copied, uploaded, Open With, and extension-command images."), QStringLiteral("仅作用于保存、复制、上传、打开方式和扩展命令图片。")},
        ...
        {QStringLiteral("Hold S to stop recording, or continue selecting a screenshot region."), QStringLiteral("按住 S 停止录制，或继续选择截图区域。")},
        ...
        {QStringLiteral("Global hotkeys are not supported on this platform. Use the tray menu or bind a desktop shortcut instead."), QStringLiteral("当前平台不支持全局热键。请使用托盘菜单或绑定桌面快捷键。")},
        {QStringLiteral("Global shortcuts portal is not available. Use the tray menu or bind a desktop shortcut instead."), QStringLiteral("当前平台未提供全局快捷键 Portal。请使用托盘菜单或绑定桌面快捷键。")},
        {QStringLiteral("No global shortcuts are configured."), QStringLiteral("未配置全局快捷键。")},
        {QStringLiteral("Failed to create global shortcut session: %1"), QStringLiteral("创建全局快捷键会话失败：%1")},
        {QStringLiteral("Global shortcuts portal returned an invalid session."), QStringLiteral("全局快捷键 Portal 返回了无效会话。")},
        {QStringLiteral("Failed to bind global shortcuts: %1"), QStringLiteral("绑定全局快捷键失败：%1")},
        {QStringLiteral("Global shortcuts portal did not bind any shortcuts."), QStringLiteral("全局快捷键 Portal 未绑定任何快捷键。")},
        {QStringLiteral("Failed to connect to global shortcut activation signal."), QStringLiteral("连接全局快捷键激活信号失败。")},
        {QStringLiteral("This key combination is not allowed as a shortcut."), QStringLiteral("该组合键不允许作为快捷键。")},
        {QStringLiteral("Press a key combination to assign, or leave empty to disable."), QStringLiteral("按下组合键以分配，或留空以禁用。")},
        {QStringLiteral("This shortcut is already assigned to another action."), QStringLiteral("该快捷键已分配给其他操作。")},
        {QStringLiteral("Global shortcuts need at least one modifier key or a function key."), QStringLiteral("全局快捷键至少需要一个修饰键或功能键。")},
        {QStringLiteral("Restore"), QStringLiteral("还原")},
        {QStringLiteral("Restore Page"), QStringLiteral("还原本页")},
        {QStringLiteral("Restore this section to the currently saved configuration."), QStringLiteral("将本栏目还原为当前已保存的配置。")},
        {QStringLiteral("Restore this page to the currently saved configuration."), QStringLiteral("将本页还原为当前已保存的配置。")},
        {QStringLiteral("Restore Original Settings"), QStringLiteral("还原原始设置")},
        {QStringLiteral("Reset every setting back to the factory defaults. This cannot be undone."), QStringLiteral("将所有设置还原为出厂默认值。此操作不可撤销。")},
        {QStringLiteral("Reset every setting to its factory default? This cannot be undone, and any unsaved changes will be lost."), QStringLiteral("是否将所有设置还原为出厂默认值？此操作不可撤销，未保存的修改将丢失。")},
        {QStringLiteral("Failed to reset settings: %1"), QStringLiteral("还原设置失败：%1")},
        {QStringLiteral("Cannot reset application config"), QStringLiteral("无法还原应用配置。")},
        {QStringLiteral("Headless Mode"), QStringLiteral("无头模式")},
        {QStringLiteral("Control where window captures go when Mark Shot runs without a UI. The clipboard is never modified unless you explicitly enable it here."), QStringLiteral("控制 Mark Shot 无界面运行时窗口截图结果的去向。除非在此显式开启，否则绝不修改剪贴板。")},
        {QStringLiteral("Default Capture Destination"), QStringLiteral("默认捕获去向")},
        {QStringLiteral("Return inline (base64, no files)"), QStringLiteral("直接返回（base64，不写文件）")},
        {QStringLiteral("Stage temporary files"), QStringLiteral("暂存临时文件")},
        {QStringLiteral("Allow Clipboard Modification"), QStringLiteral("允许修改剪贴板")},
        {QStringLiteral("Permit headless window captures to write into the system clipboard. Disabled by default; enabling requires a confirmation passphrase."), QStringLiteral("允许无头模式截图写入系统剪贴板。默认关闭；开启需输入确认口令。")},
        {QStringLiteral("Enable Headless Clipboard Access"), QStringLiteral("开启无头模式剪贴板权限")},
        {QStringLiteral("Headless mode may write into your system clipboard. To confirm you understand the risk, type the passphrase shown below exactly and press Confirm."), QStringLiteral("无头模式可能写入您的系统剪贴板。为确认您已了解风险，请逐字输入下方显示的口令并点击「确认」。")},
        {QStringLiteral("Type the passphrase"), QStringLiteral("输入口令")},
        {QStringLiteral("Confirm"), QStringLiteral("确认")},
        {QStringLiteral("Wrong Passphrase"), QStringLiteral("口令错误")},
        {QStringLiteral("The passphrase does not match. Clipboard access stays disabled."), QStringLiteral("口令不匹配，剪贴板权限保持关闭。")},
        ...
        {QStringLiteral("Italian"), QStringLiteral("意大利语")},
        {QStringLiteral("Arabic"), QStringLiteral("阿拉伯语")},
        {QStringLiteral("Portuguese"), QStringLiteral("葡萄牙语")},
        {QStringLiteral("Unsaved Changes"), QStringLiteral("未保存的修改")},
        {QStringLiteral("You have unsaved changes."), QStringLiteral("您有未保存的修改。")},
        {QStringLiteral("Save and Close"), QStringLiteral("保存并关闭")},
        {QStringLiteral("Discard and Close"), QStringLiteral("放弃并关闭")},
        {QStringLiteral("Keep Editing"), QStringLiteral("继续编辑")},
        {QStringLiteral("About"), QStringLiteral("关于")},
        {QStringLiteral("About Mark Shot"), QStringLiteral("关于 Mark Shot")},
        {QStringLiteral("Version %1"), QStringLiteral("版本 %1")},
        {QStringLiteral("Maintained by %1 and contributors."), QStringLiteral("由 %1 及贡献者维护。")},
        {QStringLiteral("Open Source Community Edition"), QStringLiteral("开源社区版")},
        {QStringLiteral("Repository"), QStringLiteral("仓库")},
        {QStringLiteral("This is the open-source community edition of Mark Shot. Source code and issue tracking: %1"), QStringLiteral("这是 Mark Shot 的开源社区版。源代码与问题跟踪：%1")},
        {QStringLiteral("Acknowledgment"), QStringLiteral("致谢")},
        {QStringLiteral("Mark Shot is based on the original upstream project %1. We sincerely thank its author and all contributors for their outstanding work."), QStringLiteral("Mark Shot 基于原上游项目 %1 开发，我们衷心感谢其作者与所有贡献者的杰出工作。")},
        {QStringLiteral("Licensed under the MIT License."), QStringLiteral("以 MIT 许可证发布。")},
        {QStringLiteral("Beijing Taiyin Zhaowu Technology Co., Ltd."), QStringLiteral("北京太殷造物科技有限公司")},
        {QStringLiteral("Discard all unsaved changes?"), QStringLiteral("是否放弃所有未保存的修改？")},
    };
    return table;
}
